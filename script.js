...
function handleResize() {
      ...
      if (window.innerWidth < 768) {
        // Для мобильных устройств добавляем отступ
        projectsSection.style.marginTop = '2rem';
      } else {
        // Для десктопа убираем лишний отступ
        projectsSection.style.marginTop = '0';
        
        // Проверяем перекрытие элементов
        const timelineRect = timeline.getBoundingClientRect();
        const projectsRect = projectsSection.getBoundingClientRect();
        
        if (timelineRect.bottom > projectsRect.top) {
          timeline.style.marginBottom = (timelineRect.bottom - projectsRect.top + 20) + 'px';
        }
      }
    }
